export default function Account({ cloud, email, count, onSync, onLogout, onReset }: { cloud: boolean; email?: string; count: number; onSync: () => void; onLogout: () => unknown; onReset: () => void }) {
  return <>
    ...
    <section className="panel settings-list"><div><span>إجمالي التكليفات</span><strong>{count}</strong></div><div><span>التخزين</span><strong>{cloud ? 'Supabase' : 'هذا الجهاز'}</strong></div><div><span>مساعد البحث</span><strong>{cloud ? 'متاح بعد إضافة المفتاح' : 'يتطلب السحابة'}</strong></div><div><span>الإصدار</span><strong>0.4.0</strong></div></section>
